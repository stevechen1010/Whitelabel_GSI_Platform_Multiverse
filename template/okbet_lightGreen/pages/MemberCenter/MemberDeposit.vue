<template>
  <HeaderTitleBack v-if="$q.platform.is.mobile" titleI18n="member.deposit.title" variant="green">
    <q-form class="deposit-layout h5" @submit="handleDepositSubmit">
      ...
      <div class="justify-center action-btns row">
        <q-btn :label="$t('common.btn.submit')" color="primary" class="submit-btn text-capitalize" type="submit" />
      </div>
    </q-form>
    <FooterNav />
  </HeaderTitleBack>
          ...
          <div class="form-content">
            ...
            <div class="form-item row" v-if="depositState.paymentDetail.extra_field_key">
              <div class="row col-12 col-sm-6">
                ...
                <div
                  class="input-content col-12 extra-input-style"
...
import { FEE_TYPE, FIELD_TYPE, GENDER, PROFILE_DETAIL_EDIT_TYPE } from "src/common/utils/constants"
...
const { userInfoForm, userWalletMap, resetUserInfoForm, setUserInfo, qDateAdultOptions } = useUserInfo()
const { memberImg, svgIcon } = useSiteImg()
const { copyMessage, genEnumToDropdown, preciseMultiply } = useCommon()
const $q = useQuasar()
const { t } = useI18n()
const Rules = useRule()
const {
...
watch(
  () => depositState.promotion_list,
  (newPromotionList) => {
    const isPromotionStillAvailable = newPromotionList.some((p) => p.id === depositState.form.promotion_id)

    if (!isPromotionStillAvailable) {
      if (newPromotionList.length > 0) {
        depositState.form.promotion_id = newPromotionList[0].id
      } else {
        depositState.form.promotion_id = 0
      }
    }
  }
)
...
.form-input
  ...
  :deep(.q-field__control)
    min-height: 40px
    height: auto
    border: 2px solid #05905F !important
