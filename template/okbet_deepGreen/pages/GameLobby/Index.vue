<template>
  ...
  <div class="provider-container" :class="`${$q.platform.is.mobile ? 'h5' : 'pc'}`">
    ...
    <swiper :slides-per-view="$q.platform.is.mobile ? 2.5 : 4.3" :space-between="10">
      <swiper-slide
        v-for="(product, key) in siteProductList"
        :key="key"
        :name="product.product_code"
        @click="handleTabClick(product.product_code)"
      >
        <img
          :src="getProductTabImage({ ...product, siteKey: 'okbet_deepGreen' })"
          class="tab-img"
...
onMounted(async () => {
  ...
  if (siteProductList.value.length) {
    handleTabClick(siteProductList.value[0].product_code)
  }
})
</script>
...
@import "app/template/okbet_deepGreen/assets/css/game.scss";
@import "app/template/okbet_deepGreen/assets/css/button.scss";
@import "app/template/okbet_deepGreen/assets/css/data.scss";
