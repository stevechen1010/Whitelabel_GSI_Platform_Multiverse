<template>
  ...
  >
    <template v-if="isMobile" #prepend>
      <q-img class="icon-coin" :src="homeImg('header/coin')" loading="lazy"></q-img>
    </template>
    <template #selected>
      <div v-if="inUseWallet" class="selected-content">
        <span v-if="isDesktop" class="mr-2 currency-code">{{ activeWalletLabel }}</span>
    ...
    <template v-slot:option="{ itemProps, opt, selected, toggleOption }">
      <q-item v-bind="itemProps" class="wallet-list wallet-bg-image">
        <q-item-section side>
          <q-checkbox :model-value="selected" @update:model-value="toggleOption(opt)" color="green" />
        </q-item-section>
...
.wallet-container {
  ...
  :deep(.q-field__inner) {
    ...
    @include phone-width {
      @apply flex items-center h-[2rem] px-0;
...
.balance {
  @apply overflow-hidden text-ellipsis text-sm font-bold;
  flex: 1;
  min-width: 0;
