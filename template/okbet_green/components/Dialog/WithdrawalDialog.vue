    ...
    <q-card>
      <q-card-section>
        <div class="dialog-title-wrapper">
          <div class="dialog-title-text">
            <div class="font-extrabold text-xl">{{ $t("menu.withdrawal") }}</div>
      ...
      <q-card-section>
        ...
        <div class="amount-wrapper">
          <div class="amount-title">{{ $t("member.deposit.withdrawAmount2") }}</div>
          <q-input
            v-model="amount"
            :rules="[
              Rules.required(),
              (val: string) => Rules.depositAmount({ min: '200', max: '100000', val })
            ]"
            borderless
            inputmode="numeric"
            pattern="[0-9]*"
            lazy-rules
            :placeholder="$t('common.withdraw_range2') + ' 200-100,000'"
          />
        </div>
