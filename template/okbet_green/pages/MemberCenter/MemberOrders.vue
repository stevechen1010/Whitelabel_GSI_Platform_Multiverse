<template>
  <HeaderTitleBack
    ...
    :title-i18n="PENDING_SEARCH_TYPE.FpI18nTitle[pendingOrderState.query.search_type]"
    ...
    <div class="h5-order-container">
      <q-form @submit="handleSubmit">
            ...
            <div class="date-input-item">
              ...
              >
                ...
                <q-menu @show="StartDatePickerShow = true" @hide="StartDatePickerShow = false">
                  <q-date v-model="pendingOrderState.query.start_date" mask="YYYY-MM-DD" color="positive" minimal />
                </q-menu>
              </q-input>
            </div>
            <div class="date-input-item right">
              ...
              >
                ...
                <q-menu @show="EndDatePickerShow = true" @hide="EndDatePickerShow = false">
                  <q-date v-model="pendingOrderState.query.end_date" mask="YYYY-MM-DD" color="positive" minimal />
                </q-menu>
              </q-input>
            </div>
          </div>
        ...
        <div class="report-content">
          ...
          <template v-if="pendingOrderState.list.length">
            <q-card v-for="(pending, index) in pendingOrderState.list" :key="index" class="report-card">
              <q-card-section class="report-card-header">
                <div class="header-title">
                  {{ $t("tableHeader.no") }} <span class="ml-2">{{ pending.trans_code }}</span>
                </div>
                <div class="header-info-row">
                  <div class="header-info">
                    <div class="header-info-label">{{ $t("tableHeader.status") }}</div>
                    <div class="header-info-value status">
                      {{ $t(PENDING_STATUS.I18nKeys[pending.status as PENDING_STATUS.Enums]) }}
                    </div>
                  </div>
                  <div class="header-info">
                    <div class="header-info-label">{{ $t("tableHeader.type") }}</div>
                    <div class="header-info-value">
                      {{ $t(PENDING_SEARCH_TYPE.FpI18nKeys[pending.order_type as PENDING_SEARCH_TYPE.Enums]) }}
                    </div>
                  </div>
                  <div class="header-info">
                    <div class="header-info-label">{{ $t("tableHeader.cashFlowType") }}</div>
                    <div class="header-info-value">
                      {{ getPaymentTypeName(pending.payment_type as FUND_METHOD_TYPE.Enums) }}
                    </div>
                  </div>
                  <div class="header-info">
                    <div class="header-info-label">{{ currencyHeader }}</div>
                    <div class="header-info-value">{{ currencyName(pending.currency) }}</div>
                  </div>
                  <div class="header-info">
                    <div class="header-info-label">{{ $t("tableHeader.amount") }}</div>
                    <div class="header-info-value amount">{{ moneyFormat(pending.amount, 2) }}</div>
                  </div>
                </div>
              </q-card-section>
              <q-card-section class="report-card-body">
                <div class="body-info-row">
                  <div class="body-info-row-label">{{ $t("tableHeader.paymentProvider") }}</div>
                  <div class="body-info-row-value">{{ pending.payment_gateway_name }}</div>
                </div>
                <div class="body-info-row">
                  <div class="body-info-row-label">{{ currencyHeader }}</div>
                  <div class="body-info-row-value">{{ currencyName(pending.currency) }}</div>
                </div>
                <div class="body-info-row">
                  <div class="body-info-row-label">{{ $t("tableHeader.amount") }}</div>
                  <div class="body-info-row-value">{{ moneyFormat(pending.amount, 2) }}</div>
                </div>
                <div class="body-info-row">
                  <div class="body-info-row-label">{{ $t("tableHeader.actualAmount") }}</div>
                  <div class="body-info-row-value">{{ moneyFormat(pending.actual_amount, 2) }}</div>
                </div>
                <div class="body-info-row">
                  <div class="body-info-row-label">{{ $t("tableHeader.dateOfApplication") }}</div>
                  <div class="body-info-row-value">
...
const searchTabs = computed(() => [
  {
    label: t(PENDING_SEARCH_TYPE.FpI18nKeys[PENDING_SEARCH_TYPE.Enums.Deposit]),
    value: PENDING_SEARCH_TYPE.Enums.Deposit
  },
  {
    label: t(PENDING_SEARCH_TYPE.FpI18nKeys[PENDING_SEARCH_TYPE.Enums.Withdrawal]),
