          ...
          <div class="info-content flex-col normal col-12 col-sm-10 w-full">
            <div class="input-content deposit-input col-12 w-full">
              <q-input
                ...
                :rules="[Rules.required(), (val) => validateAmount(val)]"
                lazy-rules
...
const handleAmountInput = (val: string) => {
  ...
  // 如果为空，保持为空字符串而不是设置为'0'
  if (cleanValue === "" || cleanValue === "0") {
    depositState.form.amount = cleanValue === "0" ? "" : cleanValue
  } else {
    depositState.form.amount = cleanValue
  }
}

const handleAmountKeypress = (e: KeyboardEvent) => {
  const key = e.key
  ...
  // 允许数字
  if (key >= "0" && key <= "9") {
    // 如果当前值是空或者光标在开头且输入的不是0，或者当前值不是以0开头
    if (
      !currentValue ||
      currentValue === "" ||
      (input.selectionStart === 0 && key !== "0") ||
...
const validateAmount = (val: string) => {
  if (!val || val === "") {
    return true // 让required rule处理空值
  }
  ...
  // 检查范围
  const min = parseFloat(depositState.paymentDetail.deposit_min || "0")
  const max = parseFloat(depositState.paymentDetail.deposit_max || "999999999")
