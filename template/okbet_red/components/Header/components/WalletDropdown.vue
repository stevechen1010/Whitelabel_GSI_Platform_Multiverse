  ...
  >
    <template #selected>
      <div v-if="inUseWallet" class="selected-content">
        ...
        <span class="balance">{{ formatWalletBalance(inUseWallet[WALLET_TYPE.Enums.Cash].balance || "0") }}</span>
      </div>
    </template>
    <template #append>
      <q-btn
        v-if="isA99M"
        flat
        class="btn-balance-abbreviation"
        @click.stop.prevent="setIsDiscardThreeDigits(!isDiscardThreeDigits)"
      >
        <q-img v-show="isDiscardThreeDigits" :src="svgIcon('btn-balance-abbreviation-open')" loading="lazy"></q-img>
        <q-img v-show="!isDiscardThreeDigits" :src="svgIcon('btn-balance-abbreviation-close')" loading="lazy"></q-img>
      </q-btn>
    ...
    <template v-slot:option="{ itemProps, opt, selected, toggleOption }">
      <q-item v-bind="itemProps" class="wallet-list wallet-bg-image">
        ...
        <q-item-section>
          <q-item-label class="pup_balance">{{ formatWalletBalance(opt.label.split(" ")[0]) }}</q-item-label>
        </q-item-section>
      </q-item>
    </template>
  </q-select>
