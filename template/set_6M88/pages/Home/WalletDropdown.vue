<template>
  <q-btn flat :ripple="false" class="no-ripple px-1">
    <div v-if="inUseWallet" class="btn-balance normal-case font-bold">
    ...
    <q-menu dark :offset="[0, 10]" anchor="bottom right" self="top right" v-if="inUseWallet">
            ...
            <div class="af-amt">
              <label v-for="(currency, key) in userWalletList" :key="key">
                <input
                  ...
                  name="example"
                  :checked="currency.currency_code === inUseWallet[WALLET_TYPE.Enums.Cash].currency_code"
                ...
                <div class="currency-amt">
                  ...
                  <span>{{ moneyFormat(currency.balance) }}</span>
                </div>
              </label>
            </div>
          </div>
...
const { userWalletList, inUseWallet, setUserActiveWallet, getUserWalletList } = useUserInfo()
