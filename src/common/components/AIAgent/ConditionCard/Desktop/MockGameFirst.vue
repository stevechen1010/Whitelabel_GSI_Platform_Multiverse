    ...
    <q-card-section>
      <div class="card-header">
        <div class="card-header-title">
          ...
          <span class="title-text">{{ $t("ai.auto_bet_setting") }} 3</span>
        </div>
      ...
      <div class="card-content">
        <div class="flex flex-col gap-1">
          ...
          <div class="product-img-wrapper">
            <q-img :src="wowGamingImg" class="product-img" />
          </div>
        </div>
        <div class="flex flex-col gap-1">
          <span class="item-title">{{ $t("ai.betting_items") }}</span>
          <div class="flex gap-1">
            <span class="item-text">TRIAL OF THE FIRE DRAGON</span>
            <span class="item-text">RTP</span>
          </div>
        </div>
      </div>
          ...
          <div class="card-disk">
            <div class="card-disk-item">
              ...
              <span v-if="condition.mode === 'RANGE'" class="item-text">
                {{ $t("ai.rtp") }} {{ $t("ai.between") }} {{ condition.min }} ~ {{ condition.max }}
              </span>
              <span v-else-if="condition.mode === 'GREATER'" class="item-text">
                {{ $t("ai.rtp") }} {{ $t("ai.greater_than") }} {{ condition.value }}
              </span>
              <span v-else-if="condition.mode === 'LESS'" class="item-text">
                {{ $t("ai.rtp") }} {{ $t("ai.less_than") }} {{ condition.value }}
              </span>
            </div>
            <div class="card-disk-item">
